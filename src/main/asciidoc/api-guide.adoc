...
This guide provides detailed description of available endpoints with the examples of their usage.
...
The user must not contain `id` field or it's value must be `null`, otherwise `422 Unprocessable Entity` error code will be returned.

The endpoint is intended to be used by unauthorized client.
--
--
* *URL*
...
Creates a dish using request body parameters and includes it in content of menu of restaurant with specified
